...
export type AutoCompleteProps<T extends AutoCompleteCandidateItem | undefined> = {
  candidates?: T[]
  renderCandidateItem?: (candidate: T, idx: number, candidates: T[]) => React.ReactNode
  renderCandidatePanelCard?: (candidate: T, idx: number, candidates: T[]) => React.ReactNode
  ...
  return (
    <Popover placement="bottom" componentRef={popoverComponentRef}>
      ...
      <Popover.Panel>
        ...
        >
          {filtered?.length ? (
            <div className="divide-y divide-[#abc4ff1a] max-h-[40vh] overflow-auto">
...
function createLabelNode<T extends string | { label?: string | undefined } | undefined>(
  candidate: T,
  idx: number,
  candidates: T[],
  ...
  return renderNode ? (
    ...
    <div className="py-2">{isString(candidate) ? candidate : candidate.label}</div>
  )
}
