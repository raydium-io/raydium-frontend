...
export default function txRemoveConcentrated({ ammId: targetAmmId }: { ammId?: PublicKeyish } = {}) {
  return handleMultiTx(async ({ transactionCollector, baseUtils: { owner, connection } }) => {
    assert(targetAmmId, 'should provide ammId to remove concentrated')

    const { getTokenAccount, tokenAccountRawInfos } = useWallet.getState()
    const { getToken } = useToken.getState()
    const { jsonInfos, currentJsonInfo, removeAmount } = useConcentrated.getState()
    ...
    assert(targetJsonInfo, `can't find target concentrated pool`)
    ...
    assert(lpTokenAccount?.publicKey, `user haven't concentrated pool's account`)
    ...
    transactionCollector.add(await loadTransaction({ transaction: transaction, signers: signers }), {
      txHistoryInfo: {
        title: 'Remove concentrated',
        description: `Remove  ${removeTokenAmount.toExact()} ${lpToken.symbol}`
      },
      onTxSuccess: () => {
        useConcentrated.setState({ removeAmount: '', isRemoveDialogOpen: false })
      }
    })
  })
}
