...
export default function useConcentratedInitCoinFiller() {
  ...
  useEffect(() => {
    setTimeout(() => {
      ...
      const isNotReady = Boolean(currentAmmPool && !coin1 && !coin2)
      if (isNotReady) return
      const queryHaveSetCoin = ['coin0', 'coin1', 'ammId'].some((i) => Object.keys(query).includes(i))
      const needFillCoin1 =
        !coin1 &&
        !currentAmmPool &&
        toPubString(coin2?.mint) !== toPubString(QuantumSOLVersionSOL.mint) &&
        !queryHaveSetCoin
      if (needFillCoin1) {
        useConcentrated.setState({ coin1: QuantumSOLVersionSOL })
      }
      const needFillCoin2 =
        !coin2 && !currentAmmPool && toPubString(coin1?.mint) !== toPubString(RAYMint) && !queryHaveSetCoin
      if (needFillCoin2) {
