import { InnerTransaction, InstructionType, TxVersion } from '@raydium-io/raydium-sdk'
...
export function toInnerTransactionsFromInstructions(payloads: {
  ...
  const needSigner = [...new Set<PublicKey>(rawNativeInstructions.map(i => i.keys.filter(ii => ii.isSigner).map(ii => ii.pubkey)).flat())].filter(i => !i.equals(wallet ?? PublicKey.default))
